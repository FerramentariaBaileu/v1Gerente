export const SYSTEM_INSTRUCTION = `INFORMAÇÕES DO SISTEMA ATUAL: O banco não possui preço de venda para produtos acabados. Não prometa consultar ou alterar esse preço. Novos produtos exigem código único, nome, categoria e unidade. O cadastro não registra estoque; use atualizar_estoque como uma operação separada. Não invente políticas ou procedimentos internos.
Você é o "Gerente Virtual" da Bailéu, um assistente interno que atua
...
export const FUNCTION_DECLARATIONS = [
  ...
  { name: 'cadastrar_produto', description: 'Cadastra um produto acabado sem estoque inicial. Exclusivo para admin.', parameters: { type: 'OBJECT', properties: { codigo: string('Código interno único.'), nome: string('Nome fantasia.'), observacoes: string('Observações.'), categoria: string('Categoria.'), unidade: string('Unidade do estoque, por exemplo un ou kg.') }, required: ['codigo', 'nome', 'categoria', 'unidade'] } },
  { name: 'atualizar_produto', description: 'Atualiza um campo permitido de um produto. Exclusivo para admin e exige confirmação anterior.', parameters: { type: 'OBJECT', properties: { produto: string('Nome ou código inequívoco.'), campo: { type: 'STRING', enum: ['fantasy_name', 'technical_name', 'description', 'category'], description: 'Campo permitido.' }, novo_valor: string('Novo valor.') }, required: ['produto', 'campo', 'novo_valor'] } }
]
